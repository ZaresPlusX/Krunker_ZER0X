...
function cripple_window(_window) {
    ...
    let patch_game_js = function(script) {
        ...
        for (let i = 0; i < whitespaces && whitespaces.length; i++) {
            call_Zares += whitespaces[i];
        }
        call_Zares += '/*';
        while (call_Zares.length < code_to_overwrite[0].length - 2) {
            call_Zares += '*';
        }
        call_Zares += '*/';

        script = script.replace(code_to_overwrite[0], call_Zares);
        script = script.slice(0, code_to_overwrite.index) + call_Zares + script.slice(code_to_overwrite.index + code_to_overwrite[0].length);
        conceal_string(code_to_overwrite[0], call_Zares);
